...
export default function DocPageLayoutMain({hiddenSidebarContainer, children}) {
  ...
  const { version: konpyutaikaVersion, EOLDate, isPrerelease, latestStableRelease } = useContext(VersionContext)
  const { pageAvailable, firstAvailableVersion } = pageVersionCheck(konpyutaikaVersion, versionedPages, currentDocRoute)
  ...
  useEffect(() => {
    ...
    if(!isPrerelease) {
      setPreData({
        ...
        setPreData({
          ...
          isPrereleaseBannerText  : `You are currently viewing v${konpyutaikaVersion}, which is a prerelease. The latest stable version is v${latestStableRelease}`
        })
    }
    ...
    if(!EOLDate) {
      ...
      if(new Date() > new Date(EOLDate)) {
        setEOLData({
          showEOLBanner: true,
          EOLBannerText: `This version is <a href="/docs/core-versions">no longer supported</a>. No patch releases will be made, even for critical security issues. For better performance, improved security, and new features, you should upgrade to ${latestStableRelease}, the latest stable version.`
        })
      } else if(new Date() > threeMonths) {
        setEOLData({
          showEOLBanner: true,
          EOLBannerText: `This version is nearing the end of its <a href="/docs/core-versions">critical support period</a>. For better performance, improved security, and new features, you should upgrade to ${latestStableRelease}, the latest stable version.`
        })
      } else {
        setEOLData({
          showEOLBanner: false,
  ...
  return (
    <main
      ...
      <div
        ...
        {!pageAvailable && konpyutaikaVersion && firstAvailableVersion && (
          <div className={styles.versionBanner}>
            <Admonition type="caution" title={`New feature!`} icon="🎉 " >
              <p style={{'marginTop': '5px', 'marginBottom': '0'}}>Unfortunately, this feature is not available in version {konpyutaikaVersion}</p>
