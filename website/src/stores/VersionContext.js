...
import { versions } from '../../konpyutaika-versions'
...
export const VersionContextProvider = ({ value = "", children }) => {
  ...
  useEffect(() => {
    const storageVersion = window.localStorage.getItem('konpyutaikaVersion')
    ...
    if(versionParam && versions.find(ver => ver?.version && ver.version === versionParam)) {
      ...
      */}
      ...
      window.localStorage.setItem('konpyutaikaVersion', versionParam)
    } else {
      {/*
      ...
      */}
      if(storageVersion && versions.find(ver => ver?.version && ver.version === storageVersion)) {
        ...
        window.localStorage.setItem('konpyutaikaVersion', lastReleasedVersion.version)
      }
    }
  }, [])
  ...
  const updateVersion = (e) => {
    ...
    versionValue &&
      ...
      window.localStorage.setItem('konpyutaikaVersion', versionValue)
  }
